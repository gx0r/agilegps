...
module.exports.view = function (ctrl, args, extras) {
    ...
    function formatVehicle(vehicle) {
        ...
        if (vehicle.obd) {
            ...
            if (obd.malfunction) {
                str += ' ';
                if (_.isFinite(obd.diagnosticTroubleCodesCount) && obd.diagnosticTroubleCodesCount > 1) {
    ...
    return m('div.business-table.fullwidth', [
        ...
        m('ul.list-group', [
            ...
            // Fleets/All
            m('li.pointer list-group-item', {
                class: state.selectedAllFleets ? 'active' : '',
                onclick: function (ev) {
                    appState.selectFleetAll();
                }
            }, require('./svg/truck-facing')(12, 12), ' '+t('Fleets/All')),

            // Individual Fleets
            fleets.map(function (fleet) {
                return [
                    m('li.pointer list-group-item', {
                        class: !state.selectedAllFleets && state.selectedFleets.length === 1 && state.selectedFleets[0].id === fleet.id ? 'active' : '',
                        onclick: function (ev) {
                            appState.selectFleet(fleet);
                        }
                    }, require('./svg/truck')(12, 12, fleet.color), ' ', m('b', fleet.name)),

                    fleet.vehicles.filter(function (vid) {
                        const vehicle = state.vehiclesByID[vid];
                        return ctrl.searchInput() === '' || vehicle.name.toUpperCase().indexOf(ctrl.searchInput().toUpperCase()) > -1;
                    }).map(function (vid) {
                        const vehicle = state.vehiclesByID[vid];
                        return m('li.pointer list-group-item', {
                            ...
                            class: state.selectedVehicle && vehicle && state.selectedVehicle.id === vehicle.id ? 'active' : '',
                            onclick: function (ev) {
