...
process.on('exit', function () {
	log.info('Cluster master? ' + cluster.isMaster + ' Process exit at ' + (new Date).toISOString());
});
...
if (config.cluster) {
	...
	if (httpServer)
		httpServer = sticky(options, httpServer);

	if (tlsServer)
		tlsServer = sticky(options, httpServer);
}

if (httpServer) {
	httpServer.listen(config.port, function () {
		log.info('Sticky cluster worker ' + (cluster.worker ? cluster.worker.id : '') + ' server listening on port ' + config.port);
	});
}

if (tlsServer) {
	tlsServer.listen(config.tlsport, function () {
		log.info('TLS Sticky cluster worker ' + (cluster.worker ? cluster.worker.id : '') + ' server listening on port ' + config.tlsport);
	});
}
