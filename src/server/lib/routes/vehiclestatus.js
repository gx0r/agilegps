...
import {cleanData, getAccuracy, nullToBlank} from '../../../common/helpers';
...
export default async (ctx, next) => {
	...
	if (ctx.query.format === 'excel') {
		...
		conf.cols = [
			...
			},
			{
				caption: 'Local Time (' + tzOffset + ' minutes offset)',
 				type:'date', width:20, beforeCellWrite: function() {
    	...
    	ctx.set("Content-Disposition", "attachment; filename=" + "ActivityReport" +  moment().format('YYYY-M-DD-HH-MM-SS') + ".xlsx");
	} else {
		ctx.body = status;
	}
}
