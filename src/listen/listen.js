...
import {port, host, sack, geocode, insertrawevents} from '../../config/listener.js';
import { isNotVerbose} from '../common/helpers';
import os from 'os';
...
(async function() {
	let cursor = await r.table('vehicles').pluck(['id', 'device']).changes({includeInitial: true}).run();
...
socket.on('listening', function() {
	...
	log.info('UDP Server listening on ' + address.address + ":" + address.port + ' ' + vehicleCache.itemCount + ' vehicles loaded');
});
...
socket.on('message', async function(message, remote) {
	...
	try {
		...
		let wrapped = new verror.WError(err, 'Error parsing from: ' + remote.address + ':' + remote.port + ' , message: ' + message);
		throw wrapped;
	}

	if (insertrawevents) {
		let raw = r.table('rawevents').insert({
			host: os.hostname(),
	...
	} else if (parsed.shorttype === 'ACK' || ['PDP', 'INF', 'CFG', 'GSM', 'VER', 'ALL'].indexOf(parsed.cmd) > -1) {
	...
	await r.table('devices').get(parsed.imei).update(deviceUpdate).run();
	...
	let id = moment.utc(parsed.senddate).format("YYYYMMDDHHmmssSSS") + '|' + parsed.imei + '|' + decimalToHex(parsed.count + '', 4); // more space efficient ID but with milliseconds
	...
	await r.table('vehicles').get(vehicle.id).replace(function(vehicle) {
		return vehicle.without(replacerObj)
	});

	await r.table('vehicles').get(vehicle.id).update(vehicleUpdate);
