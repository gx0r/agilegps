  ...
  [
    {
      type: "expand",
      message:
        "You sure you want to drop the vehicle and vehiclehistory tables?? On " +
        hostname +
        "?",
      name: "deleteTables",
      choices: [
  ...
  function(answers) {
    if (answers.deleteTables === "yes") {
      Promise.coroutine(function*() {
        yield Promise.join(
          r.tableDrop("vehicles").catch(ignore),
          r.tableDrop("devices").catch(ignore),
          r.tableDrop("vehiclehistory").catch(ignore),
          r.tableDrop("rawevents").catch(ignore)
        );
