...
export function isNotVerbose(item) {
	...
	if (item.cmd === 'FRI') {
		...
		return ! (['Sensor Motion', 'Sensor Rest', 'Fake Towing', 'Towing (FRI)'
			].indexOf(status) > -1);
...
export function mileageChange(history) {
    ...
    if (!history) {
        return []
    } else return history.reduce(function (output, current, index, array) {
...
export function ignitionMileage(history) {
    ...
    if (!history) {
        ...
        if (index === 0 || current.cmd === 'IGF' || current.cmd === 'IGN') { // first item and ignition changes reset the mileages.
...
export function addStartStop(history) {
    ...
    if (!history) {
        ...
        if (started) { // if we were moving
            if (containsMotionInformation(current)) {
                if (!isMotion(current)) { // and now we are stopped
                    started = false;
        ...
        if (!started) { // if we were stopped
            if (containsMotionInformation(current)) {
                if (isMotion(current)) { // and now we're moving
                    started = true;
...
export function rollup(history) {
    ...
    if (!history) {
        ...
        if (containsMotionInformation(current) && isIdle(current)) {
            ...
            if (later == null || (containsMotionInformation(later) && !isIdle(later))) {
        ...
        if (containsMotionInformation(current) && isPark(current)) {
            ...
            if (later == null || (containsMotionInformation(later) && !isPark(later))) {
