...
export function getStatus(item, ignoreOverride) {
	...
	if (cmd === 'FRI') {
		...
		if (cmd === 'HBM') {
			...
			return 'Harsh ' + (item.rty == '1' ? 'accel' : 'brake') + ' (' + speed + ' speed)';
		}
		...
		if (cmd == 'DOG') {
			...
			let type = item.rty;
			if (type == 1)
				return 'Watchdog reboot (timed)';
			if (type == 2)
				return 'Watchdog reboot (ign on)';
			if (type == 3)
				return 'Watchdog reboot (input triggered)';
			if (type == 4)
				return 'Watchdog reboot (GSM)';
			if (type == 5)
				return 'Watchdog reboot (GPRS)';
