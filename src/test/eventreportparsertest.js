...
test("parse GV500 MPN", function(t) {
  ...
  // This is needed in case some tests are asynchronous, so tape knows when to be done.
  const x =
    "+BUFF:GTMPN,360100,864251020143040,,,0,,,,,,,0000,0000,0000,0000,00,20160318002638,001E$";
  const parsed = parse(x);
...
test("parse GV500 STT", function(t) {
  const x =
    "+RESP:GTSTT,360100,864251020143040,,,42,0,,,,,,,0310,0260,74AF,AC5A,00,20160318002641,001F$";
  t.plan(2);
  const parsed = parse(x);
...
test("parse GV500 OPN", function(t) {
  const x =
    "+RESP:GTOPN,360100,864251020143040,,,0,,,,,,,0310,0260,74AF,AC5A,00,20160318002642,0020$";
  t.plan(2);
  const parsed = parse(x);
  t.equal(decimalToHex(parsed.count, 4), "0020");
  t.equal(parsed.vin, "");
...
test("parse buffered GV500 OPN", function(t) {
  const x =
    "+BUFF:GTOPN,360100,864251020143040,,,0,,,,,,,0310,0260,74AF,AC5A,00,20160318002642,0020$";
  t.plan(2);
  const parsed = parse(x);
  t.equal(decimalToHex(parsed.count, 4), "0020");
  t.equal(parsed.vin, "");
...
test("parse GV500 RTL", function(t) {
  const x =
    "+RESP:GTRTL,360100,864251020143040,,,,00,1,1,1.0,0,296.8,-93.296121,44.901623,20160318002705,0310,0260,74AF,AC5A,00,0.0,20160318002706,0021$";
  t.plan(2);
  const parsed = parse(x);
...
test("parse buffered STT", function(t) {
  const x =
    "+BUFF:GTSTT,360100,864251020143040,,,42,0,,,,,,,0310,0260,74AF,AC5A,00,20160318002641,001F$";
  t.plan(2);
  const parsed = parse(x);
  t.equal(decimalToHex(parsed.count, 4), "001F");
  t.equal(parsed.vin, "");
...
test("parse IGL", function(t) {
  const x =
    "+RESP:GTIGL,360100,864251020143040,,,,00,1,1,0.0,0,270.2,-93.296109,44.901704,20160318013354,0310,0260,74AF,AC5A,00,0.0,20160318013355,004D$";
  t.plan(6);
  const parsed = parse(x);
  t.equal(parsed.imei, "864251020143040");
...
test("parse OPN", function(t) {
  const x =
    "+RESP:GTOPN,360100,864251020143040,,,0,,,,,,,0310,0260,74AF,AC5A,00,20160318013256,0045$";
  t.plan(3);
  const parsed = parse(x);
  t.equal(parsed.imei, "864251020143040");
...
test("parse INF", function(t) {
  const x =
    "+BUFF:GTINF,360100,864251020143040,,gv500,42,8901260761283398892f,20,0,1,12264,,3.87,0,1,,,20160318041335,,,,,,+0000,0,20160318041337,005C$";
  const parsed = parse(x);
  t.plan(1);
...
test("parse GSM", function(t) {
  const x =
    "+BUFF:GTGSM,360100,864251020143040,,RTL,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,0310,0260,74af,ac5a,22,00,20160318042520,006C$";
  const parsed = parse(x);
  t.plan(1);
...
test("parse buffered FRI", function(t) {
  const x =
    "+BUFF:GTFRI,250506,863286020988292,,,10,1,0,0.0,27,,30.543643,50.422092,20160317205427,,,,,,16138.4,01265:18:43,,,9,110000,,,,20160318044421,AB27$";
  const parsed = parse(x);
  t.plan(18);
...
test("parse VER", function(t) {
  const x =
    "+RESP:GTVER,360201,864251020143727,,gv500,GV500,0209,0101,20160329161513,061B$";
  const parsed = parse(x);
  t.plan(2);
...
test("parse RTO", function(t) {
  const x =
    "+ACK:GTRTO,360201,864251020143727,gv500,VER,FFFF,20160329161513,061A$";
  const parsed = parse(x);
  t.plan(2);
...
test("DTT message", function(t) {
  const x =
    "+RESP:GTDTT,250403,863286020190428,,,,0,8,1235UUU,0,0.0,55,,37.534451,48.632817,20160828135149,,,,,,,,,,20160828143537,006A$";
  const parsed = parse(x);
  t.plan(3);
