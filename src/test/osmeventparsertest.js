...
test("OSM message, no failure codes", function(t) {
  const x =
    "+RESP:GTOSM,360100,864251020143727,,gv500,3,0,78FFFF,WVGZZZ1TZCW092623,1,14402,983A81C0,1605,37,86,13.8,3087,0,0,0,,15,22,,21,0,40.7,299,136.2,37.579901,48.720192,20160324142307,,,,,,0.0,20160324142309,0A98$";
...
test("parse short OSM message", function(t) {
  const x =
    "+RESP:GTOSM,360100,864251020143727,,gv500,0,0,FFFF,WVGZZZ1TZCW092623,1,14436,983A81C0,782,0,86,inf,3087,0,0,0,,11,12,,20160324142335,0A9B$";
  const parsed = parse(x);
  t.plan(4);
...
test("another OSM no failure codes", function(t) {
  const x =
    "+RESP:GTOSM,360100,864251020143727,,gv500,0,0,FFFF,WVGZZZ1TZCW092623,1,14459,983A81C0,1015,0,20,inf,3066,0,0,0,,13,18,,20160324100329,0899$";
  const parsed = parse(x);
  t.plan(22);
  t.equal(parsed.imei, "864251020143727", "imei");
  t.equal(parsed.name, "gv500", "name");
...
test("was parsing incorrect azimuth on OSM message", function(t) {
  const parsed = parse(
    "+RESP:GTOSM,360100,864251020145193,,,8,0,C0D0,1429,89,,21,94,20160406221831,01F3$"
  );
  t.plan(8);
...
test("Mazda OSM", function(t) {
  const parsed = parse(
    "+RESP:GTOSM,360100,864251020145193,,,8,0,C0D0,2039,86,15.5,78,38,20160408201518,0557$"
  );
  t.plan(8);
  t.equal(parsed.azimuth, undefined, "no azimuth");
  t.equal(parsed.reportMask, parseInt("C0D0", 16), "reportMask");
