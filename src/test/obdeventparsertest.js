...
test("OBD, 0 malfunctions, 1996 Jeep Grand Cherokee", function(t) {
  const x =
    "+RESP:GTOBD,360100,864251020143040,,g5,0,79FFFF,,1,12510,983A8000,,,,,,,,,,,,,3,0,0,0.0,82,275.8,-93.295822,44.901657,20160323230656,0310,0260,74AF,AC5A,00,82.6,20160323230658,03EB$";
  ...
  t.ok(
    _.isNaN(parsed.obd.DTCsClearedDistanceOverVehicleTotalMileage),
    "OBD DTCsClearedDistanceOverVehicleTotalMileage"
  );
  t.ok(
    _.isNaN(parsed.obd.malfunctionActivatedDistance),
    "OBD malfunctionActivatedDistance"
  );
  t.equal(parsed.obd.malfunction, false, "obd manfunction");
  t.ok(
    _.isNaN(parsed.obd.diagnosticTroubleCodesCount),
...
test("OBD, 1 malfunction, 1996 Jeep Grand Cherokee", function(t) {
  const x =
    "+RESP:GTOBD,360100,864251020143040,,g5,0,79FFFF,,1,13939,983A8000,777,0,56,inf,,,1,1,0138,12,5,,3,0,0,0.0,298,327.8,-93.296048,44.901821,20160323135025,0310,0260,74AF,AC5A,00,60.7,20160323135026,0379$";
  const parsed = parse(x);
  t.plan(28);
  ...
  t.equal(
    parsed.obd.diagnosticTroubleCodesCount,
    1,
    "OBD diagnosticTroubleCodesCount"
  );
...
test("parse OBD gv500", function(t) {
  const x =
    "+BUFF:GTOBD,360100,864251020143727,,gv500,0,FFFF,,1,12617,,,,,,,,,,,,,,20160322195018,0108$";
  const parsed = parse(x);
  t.plan(3);
...
test("long OBD message 2 failure codes", function(t) {
  const x =
    "+RESP:GTOBD,360100,864251020143040,,g5,0,79FFFF,,1,14022,983A8000,2031,102,96,3.8,,,1,2,0141,0138,25,23,,3,0,0,104.0,269,267.1,-93.269559,44.890825,20160326190646,0310,0260,74AF,A12F,00,155.6,20160326190649,05A6$";
  const parsed = parse(x);
  t.plan(30);
  t.equal(parsed.imei, "864251020143040", "imei");
  t.equal(parsed.name, "g5", "name");
  ...
  t.equal(
    parsed.obd.diagnosticTroubleCodes[0],
    "0141",
    "OBD diagnosticTroubleCode 1"
  );
  t.equal(
    parsed.obd.diagnosticTroubleCodes[1],
    "0138",
    "OBD diagnosticTroubleCode 2"
  );
...
test("2 malfunctions but missing failure codes", function(t) {
  const x =
    " +RESP:GTOBD,360100,864251020143040,,g5,0,79FFFF,,1,13880,983A8000,1734,87,95,3.6,,,1,2,,21,18,,3,0,0,88.2,228,277.9,-93.325287,44.888686,20160327010551,0310,0260,74AF,9DB3,00,168.9,20160327010553,0618$";
  const parsed = parse(x);
  t.plan(29);
  t.equal(parsed.imei, "864251020143040", "imei");
  t.equal(parsed.name, "g5", "name");
  ...
  t.equal(
    parsed.obd.diagnosticTroubleCodes.length,
    0,
    "OBD diagnosticTroubleCode length 0"
  );
...
test("fuel level input Chevy Silverado", function(t) {
  const x =
    "+RESP:GTOBD,360100,864251020143040,1GCEK19J48Z137290,g5,0,79FFFF,1GCEK19J48Z137290,1,14871,983B81E0,771,0,11,inf,24559,0,0,0,,25,29,38,6,0,0,,,,,,,0310,0260,74B1,9D83,00,278.8,20160327231033,072F$";
  const parsed = parse(x);
  t.plan(23);
  t.equal(parsed.imei, "864251020143040", "imei");
  t.equal(parsed.name, "g5", "name");
  ...
  t.equal(
    parsed.obd.DTCsClearedDistanceOverVehicleTotalMileage,
    24559,
    "OBD DTCsClearedDistanceOverVehicleTotalMileage"
  );
  t.equal(
    parsed.obd.malfunctionActivatedDistance,
    0,
    "OBD malfunctionActivatedDistance"
  );
...
test("another fuel level input Lincoln MKZ", function(t) {
  const x =
    "+RESP:GTOBD,360100,864251020143040,3LNHL2GC4BR773283,g5,0,79FFFF,3LNHL2GC4BR773283,1,14312,981B81E0,1333,0,8,,65535,0,0,0,,17,49,58,6,40976,0,0.0,0,288.9,-93.399623,44.940529,20160327232242,0310,0260,74B1,9D83,00,278.8,20160327232512,0731$";
  const parsed = parse(x);
  t.plan(29);
  t.equal(parsed.imei, "864251020143040", "imei");
  t.equal(parsed.name, "g5", "name");
...
test("Honda CRV OBD 360303", function(t) {
  const parsed = parse(
    "+RESP:GTOBD,360303,864251020143594,,gv500,0,79FFFF,,1,14125,983A8000,2682,72,91,22.9,,,1,4,1259032501710133,26,68,,3,1,0,80.2,66,65.9,37.521339,49.030879,20161017124818,,,,,,31.3,20161017124821,0150$"
  );
  t.plan(29);
  ...
  t.equal(
    parsed.obd.diagnosticTroubleCodes.length,
    4,
    "OBD diagnosticTroubleCode length 4"
  );
...
test("Honda CRV OBD new firmware", function(t) {
  const parsed = parse(
    "+RESP:GTOBD,360402,864251020143594,,gv500,0,79FFFF,,1,14360,983A8000,1709,34,87,23.4,,,1,4,1259032501710133,20,65,,3,1,0,41.6,355,73.9,37.511269,49.018711,20161018123455,,,,,,62.8,20161018123458,002C$"
  );
  t.plan(29);
  t.equal(parsed.imei, "864251020143594", "imei");
  t.equal(parsed.name, "gv500", "name");
