...
module.exports = function(commandString) {
    ...
    function loadOBDdata(startingPosition) {
        ...
        if (reportMask & bit(8))
            obd.DTCsClearedDistanceOverVehicleTotalMileage = Number.parseFloat(args[i++]);

        if (reportMask & bit(9))
            obd.malfunctionActivatedDistance = Number.parseFloat(args[i++]);

        if (reportMask & bit(10))
            obd.malfunction = args[i++] === '1';
						...
						for (let j = 0; j < codeCount; j++, i++) {
							if (args[i] && args[i] !== '') {
								...
								if (str.length === 4) {
									...
									console.error('Error parsing OBD codes, count: ' + codeCount + ' ' + args[i] + ' ' + JSON.stringify(obd.diagnosticTroubleCodes));
									break;
								}
							}
						}
        ...
        if (reportMask & bit(13))
            obd.throttlePosition = Number.parseInt(args[i++]);
        if (reportMask & bit(14))
            obd.engineLoad = Number.parseInt(args[i++])
        if (reportMask & bit(15))
            obd.fuelLevelInput = Number.parseInt(args[i++]);
        if (reportMask & bit(19)) // OBD Mileage
            obd.mileage = Number.parseFloat(args[i++]);

        if (reportMask & bit(20)) // GPS info
            i = loadGPSdata(args.length - 15);

        if (reportMask & bit(21)) // GSM info
            i = loadGSMdata(args.length - 8);

        if (reportMask & bit(22)) { // total mileage
    ...
    if (o.shorttype === 'ACK') {
        ...
        // handled by above.
    } else if (_.includes(['MPN', 'MPF', 'BTC', 'JDR'], cmd)) { // main power on, main power off, battery starts charging
        let i = loadGPSandGSMdata(3) // 3 through 13
        // args[14] reserved
    } else if (_.includes(['TOW', 'DIS', 'IOB', 'SPD', 'SOS', 'RTL', 'DOG', 'IGL', 'HBM'], cmd)) { // 3.3.1. Position Related Report
        // args[3] is reserved
        loadReportIDandType(args[4])
        o.number = args[5];
        let i = loadGPSandGSMdata(6); // 6 through 17
        o.mileage = Number.parseFloat(args[18]);
    } else if (cmd === 'FRI') { // fixed report
        o.powervcc = args[3];
        loadReportIDandType(args[4]);
        o.number = args[5];
        let i = loadGPSandGSMdata(6); // 6 through 17
    ...
    } else if (cmd === 'IGN' || cmd === 'IGF') { // ignition on, ignition off
        if (cmd === 'IGN') {
    ...
    } else if (_.includes(['STR', 'STP', 'LSP'], cmd)) { // start / stop etc page 234
        // args[3] reserved
        // args[4] reserved
        let i = loadGPSandGSMdata(5); // 5 through 16
        o.mileage = Number.parseFloat(args[17]);
    } else if (cmd === 'EPS') { // POWER low alarm, page 131
        ...
        // args[17] reserved
        o.mileage = Number.parseFloat(args[18]);
    } else if (cmd === 'GSS') { // gps signal lost, page 233
